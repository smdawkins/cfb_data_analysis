...
def combine_and_preprocess_data():
    # Combine files from S3 and preprocess
    hook = S3Hook(aws_conn_id='aws_default')
    files = hook.list_keys(bucket_name=BUCKET_NAME, prefix=CLEAN_FOLDER)
    
    data_frames = []
    for file in files:
        local_path = hook.download_file(file, bucket_name=BUCKET_NAME)
        df = pd.read_csv(local_path)
        data_frames.append(df)
    
    aggregated_data = pd.concat(data_frames, ignore_index=True)
    # Add cleaning/preprocessing steps here
...
def train_linear_model(**kwargs):
	...
	data = combine_and_preprocess_data()
