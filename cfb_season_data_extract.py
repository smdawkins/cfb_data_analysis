...
def fetch_and_upload_to_s3(endpoint, year, bucket_name, s3_key_prefix, aws_conn_id):
    ...
    api_base_url = Variable.get("data_api_url")
    api_key = Variable.get("data_api_key")  # Optionally, store the API key in a Variable
    ...
    try:
        ...
        s3_key = f"{s3_key_prefix}/{endpoint}_{year}.json"
        s3.load_file(
            filename=local_file,
            key=s3_key,
...
with DAG(
    dag_id="college_football_data_to_s3",
    ...
    def create_task(endpoint, year):
        return PythonOperator(
            ...
            op_kwargs={
                "endpoint": endpoint,
                "year": year,
                "bucket_name": "my-s3-bucket",  # Replace with your S3 bucket name
                "s3_key_prefix": "college_football_data",  # Folder in S3 bucket
                "aws_conn_id": "aws_default",  # Airflow AWS connection ID
            },
        )

    # List of endpoints and years to process
    endpoints = ["teams", "stats", "players"]
    years = [2022, 2023]
